...
impl AggregateRelation {
    pub fn new(schema: Arc<Schema>, input: Rc<RefCell<Relation>>, group_expr: Vec<CompiledExpr>, aggr_expr: Vec<CompiledAggregateExpression>) -> Self {
        AggregateRelation {
            schema,
            input,
            group_expr,
    ...
    fn accumulate_scalar(&mut self, value: &Option<ScalarValue>) -> Result<()> {
        if self.value.is_none() {
            ...
            self.value = match (&self.value, value) {
                ...
                _ => {
                    return Err(ExecutionError::ExecutionError("unsupported data type for MIN".to_string()));
                }
            }
        }
        Ok(())
    ...
    fn accumulate_scalar(&mut self, value: &Option<ScalarValue>) -> Result<()> {
        if self.value.is_none() {
            ...
            self.value = match (&self.value, value) {
                ...
                _ => {
                    return Err(ExecutionError::ExecutionError("unsupported data type for MAX".to_string()));
                }
            }
        }
        Ok(())
...
impl AggregateFunction for SumFunction {
    ...
    fn accumulate_scalar(&mut self, value: &Option<ScalarValue>) -> Result<()> {
        if self.value.is_none() {
            ...
            self.value = match (&self.value, value) {
                (Some(ScalarValue::UInt8(a)), Some(ScalarValue::UInt8(b))) => Some(ScalarValue::UInt8(*a + b)),
                (Some(ScalarValue::UInt16(a)), Some(ScalarValue::UInt16(b))) => Some(ScalarValue::UInt16(*a + b)),
                (Some(ScalarValue::UInt32(a)), Some(ScalarValue::UInt32(b))) => Some(ScalarValue::UInt32(*a + b)),
                (Some(ScalarValue::UInt64(a)), Some(ScalarValue::UInt64(b))) => Some(ScalarValue::UInt64(*a + b)),
                (Some(ScalarValue::Int8(a)), Some(ScalarValue::Int8(b))) => Some(ScalarValue::Int8(*a + b)),
                (Some(ScalarValue::Int16(a)), Some(ScalarValue::Int16(b))) => Some(ScalarValue::Int16(*a + b)),
                (Some(ScalarValue::Int32(a)), Some(ScalarValue::Int32(b))) => Some(ScalarValue::Int32(*a + b)),
                (Some(ScalarValue::Int64(a)), Some(ScalarValue::Int64(b))) => Some(ScalarValue::Int64(*a + b)),
                (Some(ScalarValue::Float32(a)), Some(ScalarValue::Float32(b))) => Some(ScalarValue::Float32(a + *b)),
                (Some(ScalarValue::Float64(a)), Some(ScalarValue::Float64(b))) => Some(ScalarValue::Float64(a + *b)),
                _ => {
                    return Err(ExecutionError::ExecutionError("unsupported data type for SUM".to_string()));
                }
            }
        }
        Ok(())
...
impl AccumulatorSet {
    ...
    fn values(&self) -> Vec<Option<ScalarValue>> {
        self.aggr_values.iter().map(|x| x.borrow().result().clone()).collect()
    }
}
...
fn create_accumulators(aggr_expr: &Vec<CompiledAggregateExpression>) -> Result<AccumulatorSet> {
    let aggr_values: Vec<Rc<RefCell<AggregateFunction>>> = aggr_expr
        .iter()
        .map(|e| match e.aggr_type() {
            AggregateType::Min => Ok(Rc::new(RefCell::new(MinFunction::new(e.data_type()))) as Rc<RefCell<AggregateFunction>>),
            AggregateType::Max => Ok(Rc::new(RefCell::new(MaxFunction::new(e.data_type()))) as Rc<RefCell<AggregateFunction>>),
            AggregateType::Sum => Ok(Rc::new(RefCell::new(SumFunction::new(e.data_type()))) as Rc<RefCell<AggregateFunction>>),
            _ => Err(ExecutionError::ExecutionError("unsupported aggregate function".to_string())),
        })
...
fn array_min(array: ArrayRef, dt: &DataType) -> Result<Option<ScalarValue>> {
    match dt {
        DataType::UInt8 => match compute::min(array.as_any().downcast_ref::<UInt8Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt8(n))),
            None => Ok(None),
        },
        DataType::UInt16 => match compute::min(array.as_any().downcast_ref::<UInt16Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt16(n))),
            None => Ok(None),
        },
        DataType::UInt32 => match compute::min(array.as_any().downcast_ref::<UInt32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt32(n))),
            None => Ok(None),
        },
        DataType::UInt64 => match compute::min(array.as_any().downcast_ref::<UInt64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt64(n))),
            None => Ok(None),
        },
        DataType::Int8 => match compute::min(array.as_any().downcast_ref::<Int8Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int8(n))),
            None => Ok(None),
        },
        DataType::Int16 => match compute::min(array.as_any().downcast_ref::<Int16Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int16(n))),
            None => Ok(None),
        },
        DataType::Int32 => match compute::min(array.as_any().downcast_ref::<Int32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int32(n))),
            None => Ok(None),
        },
        DataType::Int64 => match compute::min(array.as_any().downcast_ref::<Int64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int64(n))),
            None => Ok(None),
        },
        DataType::Float32 => match compute::min(array.as_any().downcast_ref::<Float32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Float32(n))),
            None => Ok(None),
        },
        DataType::Float64 => match compute::min(array.as_any().downcast_ref::<Float64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Float64(n))),
            None => Ok(None),
        },
        _ => Err(ExecutionError::ExecutionError("Unsupported data type for MIN".to_string())),
    }
}

fn array_max(array: ArrayRef, dt: &DataType) -> Result<Option<ScalarValue>> {
    match dt {
        DataType::UInt8 => match compute::max(array.as_any().downcast_ref::<UInt8Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt8(n))),
            None => Ok(None),
        },
        DataType::UInt16 => match compute::max(array.as_any().downcast_ref::<UInt16Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt16(n))),
            None => Ok(None),
        },
        DataType::UInt32 => match compute::max(array.as_any().downcast_ref::<UInt32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt32(n))),
            None => Ok(None),
        },
        DataType::UInt64 => match compute::max(array.as_any().downcast_ref::<UInt64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt64(n))),
            None => Ok(None),
        },
        DataType::Int8 => match compute::max(array.as_any().downcast_ref::<Int8Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int8(n))),
            None => Ok(None),
        },
        DataType::Int16 => match compute::max(array.as_any().downcast_ref::<Int16Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int16(n))),
            None => Ok(None),
        },
        DataType::Int32 => match compute::max(array.as_any().downcast_ref::<Int32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int32(n))),
            None => Ok(None),
        },
        DataType::Int64 => match compute::max(array.as_any().downcast_ref::<Int64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int64(n))),
            None => Ok(None),
        },
        DataType::Float32 => match compute::max(array.as_any().downcast_ref::<Float32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Float32(n))),
            None => Ok(None),
        },
        DataType::Float64 => match compute::max(array.as_any().downcast_ref::<Float64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Float64(n))),
            None => Ok(None),
        },
        _ => Err(ExecutionError::ExecutionError("Unsupported data type for MAX".to_string())),
    }
}

fn array_sum(array: ArrayRef, dt: &DataType) -> Result<Option<ScalarValue>> {
    match dt {
        DataType::UInt8 => match compute::sum(array.as_any().downcast_ref::<UInt8Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt8(n))),
            None => Ok(None),
        },
        DataType::UInt16 => match compute::sum(array.as_any().downcast_ref::<UInt16Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt16(n))),
            None => Ok(None),
        },
        DataType::UInt32 => match compute::sum(array.as_any().downcast_ref::<UInt32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt32(n))),
            None => Ok(None),
        },
        DataType::UInt64 => match compute::sum(array.as_any().downcast_ref::<UInt64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::UInt64(n))),
            None => Ok(None),
        },
        DataType::Int8 => match compute::sum(array.as_any().downcast_ref::<Int8Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int8(n))),
            None => Ok(None),
        },
        DataType::Int16 => match compute::sum(array.as_any().downcast_ref::<Int16Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int16(n))),
            None => Ok(None),
        },
        DataType::Int32 => match compute::sum(array.as_any().downcast_ref::<Int32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int32(n))),
            None => Ok(None),
        },
        DataType::Int64 => match compute::sum(array.as_any().downcast_ref::<Int64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Int64(n))),
            None => Ok(None),
        },
        DataType::Float32 => match compute::sum(array.as_any().downcast_ref::<Float32Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Float32(n))),
            None => Ok(None),
        },
        DataType::Float64 => match compute::sum(array.as_any().downcast_ref::<Float64Array>().unwrap()) {
            Some(n) => Ok(Some(ScalarValue::Float64(n))),
            None => Ok(None),
        },
        _ => Err(ExecutionError::ExecutionError("Unsupported data type for SUM".to_string())),
    }
}

fn update_accumulators(batch: &RecordBatch, row: usize, accumulator_set: &mut AccumulatorSet, aggr_expr: &Vec<CompiledAggregateExpression>) -> Result<()> {
    ...
    for j in 0..accumulator_set.aggr_values.len() {
        ...
        let value: Option<ScalarValue> = match aggr_expr[j].data_type() {
            ...
            other => {
                return Err(ExecutionError::ExecutionError(format!("Unsupported data type {:?} for result of aggregate expression", other)));
            }
        };
...
impl Iterator for AggregateRelation {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.end_of_results {
            ...
            let result = if self.group_expr.is_empty() { self.without_group_by() } else { self.with_group_by() };
...
macro_rules! array_from_scalar {
    ($BUILDER:ident, $TY:ident, $ACCUM:expr) => {{
        ...
        if err {
            Err(ExecutionError::ExecutionError("unexpected type when creating array from scalar value".to_string()))
        } else {
...
macro_rules! group_array_from_map_entries {
    ($BUILDER:ident, $TY:ident, $ENTRIES:expr, $COL_INDEX:expr) => {{
        ...
        if err {
            Err(ExecutionError::ExecutionError("unexpected type when creating array from aggregate map".to_string()))
        } else {
...
impl AggregateRelation {
    ...
    fn without_group_by(&mut self) -> Result<Option<RecordBatch>> {
        ...
        while let Some(batch) = self.input.borrow_mut().next() {
            for i in 0..aggr_expr_count {
                ...
                match self.aggr_expr[i].aggr_type() {
                    ...
                    AggregateType::Sum => accumulator_set.accumulate_scalar(i, array_sum(array, &t)?)?,
                    _ => {
                        return Err(ExecutionError::NotImplemented("Unsupported aggregate function".to_string()));
                    }
                }
            }
        }
        ...
        for i in 0..aggr_expr_count {
            ...
            match accum.data_type() {
                DataType::UInt8 => result_columns.push(array_from_scalar!(UInt8Builder, UInt8, accum)?),
                DataType::UInt16 => result_columns.push(array_from_scalar!(UInt16Builder, UInt16, accum)?),
                DataType::UInt32 => result_columns.push(array_from_scalar!(UInt32Builder, UInt32, accum)?),
                DataType::UInt64 => result_columns.push(array_from_scalar!(UInt64Builder, UInt64, accum)?),
                DataType::Int8 => result_columns.push(array_from_scalar!(Int8Builder, Int8, accum)?),
                DataType::Int16 => result_columns.push(array_from_scalar!(Int16Builder, Int16, accum)?),
                DataType::Int32 => result_columns.push(array_from_scalar!(Int32Builder, Int32, accum)?),
                DataType::Int64 => result_columns.push(array_from_scalar!(Int64Builder, Int64, accum)?),
                DataType::Float32 => result_columns.push(array_from_scalar!(Float32Builder, Float32, accum)?),
                DataType::Float64 => result_columns.push(array_from_scalar!(Float64Builder, Float64, accum)?),
                _ => return Err(ExecutionError::NotImplemented("tbd".to_string())),
            }
        }

        Ok(Some(RecordBatch::try_new(self.schema.clone(), result_columns)?))
    }
    ...
    fn with_group_by(&mut self) -> Result<Option<RecordBatch>> {
        ...
        let mut map: FnvHashMap<Vec<GroupByScalar>, Rc<RefCell<AccumulatorSet>>> = FnvHashMap::default();
        ...
        while let Some(batch) = self.input.borrow_mut().next() {
            ...
            let group_by_keys: Vec<ArrayRef> = self.group_expr.iter().map(|e| e.invoke(&batch)).collect::<Result<Vec<ArrayRef>>>()?;
                ...
                let key: Vec<GroupByScalar> = group_by_keys
                    ...
                    .map(|col| match col.data_type() {
                        ...
                        DataType::Utf8 => {
                            ...
                            Ok(GroupByScalar::Utf8(String::from(str::from_utf8(array.value(row)).unwrap())))
                        }
                        _ => Err(ExecutionError::ExecutionError("Unsupported GROUP BY data type".to_string())),
                    })
        ...
        let entries: Vec<MapEntry> = map
            ...
            .map(|(k, v)| {
                let x = v.borrow();
                MapEntry { k: k.clone(), v: x.values() }
            })
            .collect();

        // build the result arrays
        let mut result_arrays: Vec<ArrayRef> = Vec::with_capacity(self.group_expr.len() + self.aggr_expr.len());
        ...
        for i in 0..self.group_expr.len() {
            let array: Result<ArrayRef> = match self.group_expr[i].data_type() {
                ...
                DataType::UInt16 => group_array_from_map_entries!(UInt16Builder, UInt16, entries, i),
                DataType::UInt32 => group_array_from_map_entries!(UInt32Builder, UInt32, entries, i),
                DataType::UInt64 => group_array_from_map_entries!(UInt64Builder, UInt64, entries, i),
                ...
                _ => Err(ExecutionError::ExecutionError("Unsupported group by expr".to_string())),
            };
        ...
        for i in 0..self.aggr_expr.len() {
            let array = match self.aggr_expr[i].data_type() {
                ...
                DataType::Float32 => aggr_array_from_map_entries!(Float32Builder, Float32, entries, i),
                DataType::Float64 => aggr_array_from_map_entries!(Float64Builder, Float64, entries, i),
                _ => Err(ExecutionError::ExecutionError("Unsupported aggregate expr".to_string())),
            };
            result_arrays.push(array?);
        }

        Ok(Some(RecordBatch::try_new(self.schema.clone(), result_arrays)?))
    }
}
...
mod tests {
    ...
    fn min_f64_group_by_string() {
        ...
        let relation = load_csv(&format!("{}/csv/aggregate_test_100.csv", testdata), &schema);
    ...
    fn test_min_max_sum_f64_group_by_uint32() {
        ...
        let group_by_expr = expression::compile_expr(&context, &Expr::Column(1), &schema).unwrap();
        ...
        let mut projection = AggregateRelation::new(aggr_schema, relation, vec![group_by_expr], vec![min_expr, max_expr, sum_expr]);
        let batch = projection.next().unwrap().unwrap();
        ...
        let sum = batch.column(3).as_any().downcast_ref::<Float64Array>().unwrap();
    ...
    fn load_csv(filename: &str, schema: &Arc<Schema>) -> Rc<RefCell<Relation>> {
        ...
        Rc::new(RefCell::new(DataSourceRelation::new(Arc::new(Mutex::new(ds)))))
    }
