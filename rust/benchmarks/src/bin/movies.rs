...
fn main() -> Result<()> {
    ...
    match opt.file_format.as_str() {
        "csv" => {
            let schema = ratings_schema();
            // let options = CsvReadOptions::new().schema(&schema).has_header(true);
            let options = CsvReadOptions::new().has_header(true);
            ctx.register_csv("ratings", path, options)?
        }
        "parquet" => ctx.register_parquet("ratings", path)?,
        other => {
...
fn datafusion_sql_benchmarks(
    ...
    queries.insert("avg_rating_by_user", "SELECT userId, AVG(rating) FROM ratings GROUP BY userId");
