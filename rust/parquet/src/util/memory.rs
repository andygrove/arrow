...
impl Mem {
    pub fn new(current_mem: i64,
               max_mem: i64) -> Self {
        Self { current_mem, max_mem }
    }
}
...
impl MemTracker {
    ...
    pub fn alloc(&self, num_bytes: i64) {
        ...
        let (current, mut maximum) = (mem.current_mem, mem.max_mem);
        let new_current = current + num_bytes;
        if new_current > maximum {
            maximum = new_current
        }
        mem.current_mem = new_current;
        mem.max_mem = maximum;
    }
}
