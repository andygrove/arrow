...
pub fn get_column_reader(col_descr: ColumnDescPtr, col_page_reader: Box<PageReader>) -> ColumnReader {
    match col_descr.physical_type() {
        ...
        Type::FIXED_LEN_BYTE_ARRAY => ColumnReader::FixedLenByteArrayColumnReader(ColumnReaderImpl::new(col_descr, col_page_reader)),
    }
}
...
impl<T: DataType> ColumnReaderImpl<T> {
    ...
    #[inline]
    pub fn read_batch(&mut self, batch_size: usize, mut def_levels: Option<&mut [i16]>, mut rep_levels: Option<&mut [i16]>, values: &mut [T::T]) -> Result<(usize, usize)> {
        ...
        while max(values_read, levels_read) < batch_size {
            ...
            let iter_batch_size = {
                ...
                let mut adjusted_size = min(batch_size, (self.num_buffered_values - self.num_decoded_values) as usize);
            ...
            if self.descr.max_def_level() > 0 && def_levels.as_ref().is_some() {
                if let Some(ref mut levels) = def_levels {
                    num_def_levels = self.read_def_levels(&mut levels[levels_read..levels_read + iter_batch_size])?;
            ...
            if self.descr.max_rep_level() > 0 && rep_levels.is_some() {
                if let Some(ref mut levels) = rep_levels {
                    num_rep_levels = self.read_rep_levels(&mut levels[levels_read..levels_read + iter_batch_size])?;
                    ...
                    match current_page {
                        ...
                        Page::DataPage {
                            ...
                            if self.descr.max_rep_level() > 0 {
                                let mut rep_decoder = LevelDecoder::v1(rep_level_encoding, self.descr.max_rep_level());
                            ...
                            if self.descr.max_def_level() > 0 {
                                let mut def_decoder = LevelDecoder::v1(def_level_encoding, self.descr.max_def_level());
                        ...
                        Page::DataPageV2 {
                            ...
                            if self.descr.max_rep_level() > 0 {
                                ...
                                let bytes_read = rep_decoder.set_data_range(self.num_buffered_values as usize, &buf, offset, rep_levels_byte_len as usize);
                                offset += bytes_read;
                                self.rep_level_decoder = Some(rep_decoder);
                            }
                            ...
                            if self.descr.max_def_level() > 0 {
                                ...
                                let bytes_read = def_decoder.set_data_range(self.num_buffered_values as usize, &buf, offset, def_levels_byte_len as usize);
                                offset += bytes_read;
                                self.def_level_decoder = Some(def_decoder);
                            }
    ...
    fn set_current_page_encoding(&mut self, mut encoding: Encoding, buffer_ptr: &ByteBufferPtr, offset: usize, len: usize) -> Result<()> {
    ...
    fn read_values(&mut self, buffer: &mut [T::T]) -> Result<usize> {
        ...
        let current_decoder = self.decoders.get_mut(&encoding).expect(format!("decoder for encoding {} should be set", encoding).as_str());
...
mod tests {
    ...
    macro_rules! test {
     ...
     $num_pages:expr, $num_levels:expr, $batch_size:expr, $min:expr, $max:expr) => {
            test_internal!($test_func, Int32Type, get_test_int32_type, $func, $def_level, $rep_level, $num_pages, $num_levels, $batch_size, $min, $max);
        };
        // branch for generating i64 cases
        ($test_func:ident, i64, $func:ident, $def_level:expr, $rep_level:expr,
     $num_pages:expr, $num_levels:expr, $batch_size:expr, $min:expr, $max:expr) => {
            test_internal!($test_func, Int64Type, get_test_int64_type, $func, $def_level, $rep_level, $num_pages, $num_levels, $batch_size, $min, $max);
        };
    }
    ...
    macro_rules! test_internal {
     ...
     $min:expr, $max:expr) => {
            #[test]
            fn $test_func() {
                let desc = Rc::new(ColumnDescriptor::new(Rc::new($pty()), None, $def_level, $rep_level, ColumnPath::new(Vec::new())));
    ...
    test!(test_read_plain_v1_int32, i32, plain_v1, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 16, ::std::i32::MIN, ::std::i32::MAX);
    test!(test_read_plain_v2_int32, i32, plain_v2, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 16, ::std::i32::MIN, ::std::i32::MAX);

    test!(test_read_plain_v1_int32_uneven, i32, plain_v1, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 17, ::std::i32::MIN, ::std::i32::MAX);
    test!(test_read_plain_v2_int32_uneven, i32, plain_v2, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 17, ::std::i32::MIN, ::std::i32::MAX);

    test!(test_read_plain_v1_int32_multi_page, i32, plain_v1, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 512, ::std::i32::MIN, ::std::i32::MAX);
    test!(test_read_plain_v2_int32_multi_page, i32, plain_v2, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 512, ::std::i32::MIN, ::std::i32::MAX);

    // test cases when column descriptor has MAX_DEF_LEVEL = 0 and MAX_REP_LEVEL = 0
    test!(test_read_plain_v1_int32_required_non_repeated, i32, plain_v1, 0, 0, NUM_PAGES, NUM_LEVELS, 16, ::std::i32::MIN, ::std::i32::MAX);
    test!(test_read_plain_v2_int32_required_non_repeated, i32, plain_v2, 0, 0, NUM_PAGES, NUM_LEVELS, 16, ::std::i32::MIN, ::std::i32::MAX);

    test!(test_read_plain_v1_int64, i64, plain_v1, 1, 1, NUM_PAGES, NUM_LEVELS, 16, ::std::i64::MIN, ::std::i64::MAX);
    test!(test_read_plain_v2_int64, i64, plain_v2, 1, 1, NUM_PAGES, NUM_LEVELS, 16, ::std::i64::MIN, ::std::i64::MAX);

    test!(test_read_plain_v1_int64_uneven, i64, plain_v1, 1, 1, NUM_PAGES, NUM_LEVELS, 17, ::std::i64::MIN, ::std::i64::MAX);
    test!(test_read_plain_v2_int64_uneven, i64, plain_v2, 1, 1, NUM_PAGES, NUM_LEVELS, 17, ::std::i64::MIN, ::std::i64::MAX);

    test!(test_read_plain_v1_int64_multi_page, i64, plain_v1, 1, 1, NUM_PAGES, NUM_LEVELS, 512, ::std::i64::MIN, ::std::i64::MAX);
    test!(test_read_plain_v2_int64_multi_page, i64, plain_v2, 1, 1, NUM_PAGES, NUM_LEVELS, 512, ::std::i64::MIN, ::std::i64::MAX);

    // test cases when column descriptor has MAX_DEF_LEVEL = 0 and MAX_REP_LEVEL = 0
    test!(test_read_plain_v1_int64_required_non_repeated, i64, plain_v1, 0, 0, NUM_PAGES, NUM_LEVELS, 16, ::std::i64::MIN, ::std::i64::MAX);
    test!(test_read_plain_v2_int64_required_non_repeated, i64, plain_v2, 0, 0, NUM_PAGES, NUM_LEVELS, 16, ::std::i64::MIN, ::std::i64::MAX);

    test!(test_read_dict_v1_int32_small, i32, dict_v1, MAX_DEF_LEVEL, MAX_REP_LEVEL, 2, 2, 16, 0, 3);
    test!(test_read_dict_v2_int32_small, i32, dict_v2, MAX_DEF_LEVEL, MAX_REP_LEVEL, 2, 2, 16, 0, 3);

    test!(test_read_dict_v1_int32, i32, dict_v1, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 16, 0, 3);
    test!(test_read_dict_v2_int32, i32, dict_v2, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 16, 0, 3);

    test!(test_read_dict_v1_int32_uneven, i32, dict_v1, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 17, 0, 3);
    test!(test_read_dict_v2_int32_uneven, i32, dict_v2, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 17, 0, 3);

    test!(test_read_dict_v1_int32_multi_page, i32, dict_v1, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 512, 0, 3);
    test!(test_read_dict_v2_int32_multi_page, i32, dict_v2, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 512, 0, 3);

    test!(test_read_dict_v1_int64, i64, dict_v1, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 16, 0, 3);
    test!(test_read_dict_v2_int64, i64, dict_v2, MAX_DEF_LEVEL, MAX_REP_LEVEL, NUM_PAGES, NUM_LEVELS, 16, 0, 3);
    ...
    fn test_read_batch_different_buf_sizes() {
        test_read_batch_int32(16, &mut vec![0; 8], Some(&mut vec![0; 9]), Some(&mut vec![0; 7]));
        test_read_batch_int32(16, &mut vec![0; 1], Some(&mut vec![0; 9]), Some(&mut vec![0; 3]));
    }

    #[test]
    fn test_read_batch_values_def_rep_levels() {
        test_read_batch_int32(128, &mut vec![0; 128], Some(&mut vec![0; 128]), Some(&mut vec![0; 128]));
    }
    ...
    fn test_read_batch_adjust_after_buffering_page() {
        ...
        let desc = Rc::new(ColumnDescriptor::new(Rc::new(primitive_type), None, 1, 1, ColumnPath::new(Vec::new())));
        ...
        tester.test_read_batch(desc, Encoding::RLE_DICTIONARY, num_pages, num_levels, batch_size, ::std::i32::MIN, ::std::i32::MAX, values, Some(def_levels), Some(rep_levels), false);
    }
    ...
    // - `test_read_batch_int32`, wrapper for `read_batch` tests, since they are basically the same, just different def/rep levels and batch size.
    //
    // # Page assembly
    //
    ...
    fn test_read_batch_int32(batch_size: usize, values: &mut [i32], def_levels: Option<&mut [i16]>, rep_levels: Option<&mut [i16]>) {
        let primitive_type = get_test_int32_type();
        ...
        let desc = Rc::new(ColumnDescriptor::new(Rc::new(primitive_type), None, max_def_level, max_rep_level, ColumnPath::new(Vec::new())));
        let mut tester = ColumnReaderTester::<Int32Type>::new();
        tester.test_read_batch(desc, Encoding::RLE_DICTIONARY, NUM_PAGES, NUM_LEVELS, batch_size, ::std::i32::MIN, ::std::i32::MAX, values, def_levels, rep_levels, false);
    }
        ...
        T::T: PartialOrd + SampleRange + Copy,
    {
        ...
        fn plain_v1(&mut self, desc: ColumnDescPtr, num_pages: usize, num_levels: usize, batch_size: usize, min: T::T, max: T::T) {
            self.test_read_batch_general(desc, Encoding::PLAIN, num_pages, num_levels, batch_size, min, max, false);
        }

        // Method to generate and test data pages v2
        fn plain_v2(&mut self, desc: ColumnDescPtr, num_pages: usize, num_levels: usize, batch_size: usize, min: T::T, max: T::T) {
            self.test_read_batch_general(desc, Encoding::PLAIN, num_pages, num_levels, batch_size, min, max, true);
        }

        // Method to generate and test dictionary page + data pages v1
        fn dict_v1(&mut self, desc: ColumnDescPtr, num_pages: usize, num_levels: usize, batch_size: usize, min: T::T, max: T::T) {
            self.test_read_batch_general(desc, Encoding::RLE_DICTIONARY, num_pages, num_levels, batch_size, min, max, false);
        }

        // Method to generate and test dictionary page + data pages v2
        fn dict_v2(&mut self, desc: ColumnDescPtr, num_pages: usize, num_levels: usize, batch_size: usize, min: T::T, max: T::T) {
            self.test_read_batch_general(desc, Encoding::RLE_DICTIONARY, num_pages, num_levels, batch_size, min, max, true);
        }
        ...
        fn test_read_batch_general(&mut self, desc: ColumnDescPtr, encoding: Encoding, num_pages: usize, num_levels: usize, batch_size: usize, min: T::T, max: T::T, use_v2: bool) {
            ...
            self.test_read_batch(desc, encoding, num_pages, num_levels, batch_size, min, max, &mut values, Some(&mut def_levels), Some(&mut rep_levels), use_v2);
        }

        // Helper function to test `read_batch()` method with custom buffers for values,
        // definition and repetition levels.
        fn test_read_batch(&mut self, desc: ColumnDescPtr, encoding: Encoding, num_pages: usize, num_levels: usize, batch_size: usize, min: T::T, max: T::T, values: &mut [T::T], mut def_levels: Option<&mut [i16]>, mut rep_levels: Option<&mut [i16]>, use_v2: bool) {
            let mut pages = VecDeque::new();
            make_pages::<T>(desc.clone(), encoding, num_pages, num_levels, min, max, &mut self.def_levels, &mut self.rep_levels, &mut self.values, &mut pages, use_v2);
            ...
            while !done {
                ...
                let (values_read, levels_read) = typed_column_reader.read_batch(batch_size, actual_def_levels, actual_rep_levels, &mut values[curr_values_read..]).expect("read_batch() should be OK");
            ...
            assert_eq!(&values[0..curr_values_read], &self.values[0..curr_values_read], "values content doesn't match");

            if let Some(ref levels) = def_levels {
                assert!(levels.len() >= curr_levels_read, "def_levels.len() >= levels_read");
                assert_eq!(&levels[0..curr_levels_read], &self.def_levels[0..curr_levels_read], "definition levels content doesn't match");
            }

            if let Some(ref levels) = rep_levels {
                assert!(levels.len() >= curr_levels_read, "rep_levels.len() >= levels_read");
                assert_eq!(&levels[0..curr_levels_read], &self.rep_levels[0..curr_levels_read], "repetition levels content doesn't match");
            }

            if def_levels.is_none() && rep_levels.is_none() {
                assert!(curr_levels_read == 0, "expected to read 0 levels, found {}", curr_levels_read);
            } else if def_levels.is_some() && max_def_level > 0 {
                assert!(curr_levels_read >= curr_values_read, "expected levels read to be greater than values read");
            }
        }
    }
    ...
    impl DataPageBuilder for DataPageBuilderImpl {
        ...
        fn add_values<T: DataType>(&mut self, encoding: Encoding, values: &[T::T]) {
            assert!(self.num_values >= values.len() as u32, "num_values: {}, values.len(): {}", self.num_values, values.len());
            self.encoding = Some(encoding);
            let mut encoder: Box<Encoder<T>> = get_encoder::<T>(self.desc.clone(), encoding, self.mem_tracker.clone()).expect("get_encoder() should be OK");
    ...
    fn make_pages<T: DataType>(desc: ColumnDescPtr, encoding: Encoding, num_pages: usize, levels_per_page: usize, min: T::T, max: T::T, def_levels: &mut Vec<i16>, rep_levels: &mut Vec<i16>, values: &mut Vec<T::T>, pages: &mut VecDeque<Page>, use_v2: bool)
    where
        T::T: PartialOrd + SampleRange + Copy,
    {
