...
use crate::execution_plans::{QueryStageExec, UnresolvedShuffleExec, ShuffleReaderExec};
    ...
    let operator_str =
        if let Some(exec) = plan.as_any().downcast_ref::<HashAggregateExec>() {
            ...
            format!(
                ...
                exec.partition_location.iter().map(|p| format!("{:?}", p.partition_id)).collect::<Vec<String>>().join(",")
            )
