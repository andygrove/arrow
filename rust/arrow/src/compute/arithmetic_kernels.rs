...
/// `null`.
pub fn math_op<T, F>(left: &PrimitiveArray<T>, right: &PrimitiveArray<T>, op: F) -> Result<PrimitiveArray<T>>
where
...
fn simd_math_op<T, F>(left: &PrimitiveArray<T>, right: &PrimitiveArray<T>, op: F) -> Result<PrimitiveArray<T>>
where
    T: datatypes::ArrowNumericType,
    T::Simd: Add<Output = T::Simd> + Sub<Output = T::Simd> + Mul<Output = T::Simd> + Div<Output = T::Simd>,
    F: Fn(T::Simd, T::Simd) -> T::Simd,
{
    ...
    let null_bit_buffer = apply_bin_op_to_option_bitmap(left.data().null_bitmap(), right.data().null_bitmap(), |a, b| a & b)?;
    ...
    for i in (0..left.len()).step_by(lanes) {
        ...
        let result_slice: &mut [T::Native] = unsafe { from_raw_parts_mut((result.data_mut().as_mut_ptr() as *mut T::Native).offset(i as isize), lanes) };
        T::write(simd_result, result_slice);
    }

    let data = ArrayData::new(T::get_data_type(), left.len(), None, null_bit_buffer, left.offset(), vec![result.freeze()], vec![]);
...
where
    ...
    T::Native: Add<Output = T::Native> + Sub<Output = T::Native> + Mul<Output = T::Native> + Div<Output = T::Native> + Zero,
{
    math_op(left, right, |a, b| if b.is_zero() { Err(ArrowError::DivideByZero) } else { Ok(a / b) })
}
...
mod tests {
    ...
    fn test_primitive_array_add_mismatched_length() {
        ...
        let e = add(&a, &b).err().expect("should have failed due to different lengths");
    ...
    fn test_primitive_array_divide_by_zero() {
        ...
        assert_eq!(ArrowError::DivideByZero, divide(&a, &b).err().expect("divide by zero should fail"));
    }
